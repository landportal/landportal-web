...
namespace Drupal\issue_projects\Plugin\Block;

use Drupal\taxonomy\Entity\Term;
use Drupal\landportal_remote_block\Plugin\RemoteBlockBase;

/**
 * Provides a 'Issue projects' block.
 *
 * @Block(
 *   id = "region_issue_projects_block",
 *   admin_label = @Translation("Region issue projects block"),
 *   category = @Translation("Landportal Remote")
 * )
 */
class IssueProjectsBlock extends RemoteBlockBase {
  ...
  public function build() {
    ...
    if (count($items)) {
      $build = [
        '#theme' => 'issue_projects_block',
        '#items' => $items,
        //'#more_url' => $this->getLanguagePrefix() . 'issues/' . $this->taxonomyTerm . '/project',
        '#more_url' => $this->getLanguagePrefix() . 'search-in-projects?key=&f[0]=by_issue:' . $this->taxonomyTerm,
