...
namespace Drupal\issue_organizations\Plugin\Block;

use Drupal\landportal_remote_block\Plugin\RemoteBlockBase;
use Drupal\taxonomy\Entity\Term;

/**
 * Provides a 'Issue organizations' block.
 *
 * @Block(
 *   id = "issue_organizations_block",
 *   admin_label = @Translation("Issue organizations block"),
 *   category = @Translation("Landportal Remote")
 * )
 */
class IssueOrganizationsBlock extends RemoteBlockBase {
  ...
  public function build() {
    ...
    if (count($items)) {
      $build = [
        '#theme' => 'issue_organizations_block',
        '#items' => $items,
        '#more_url' => $this->getLanguagePrefix() . 'issues/' . $this->taxonomyTerm . '/organization',
