...
namespace Drupal\issue_library_resources\Plugin\Block;

use Drupal\landportal_remote_block\Plugin\RemoteBlockBase;
use Drupal\taxonomy\Entity\Term;

/**
 * Provides a 'Issue library resources' block.
 *
 * @Block(
 *   id = "issue_library_resources_block",
 *   admin_label = @Translation("Issue library resources block"),
 *   category = @Translation("Landportal Remote")
 * )
 */
class IssueLibraryResourcesBlock extends RemoteBlockBase {
  ...
  public function build() {
    ...
    if (count($items)) {
      $build = [
        '#theme' => 'issue_library_resources_block',
        '#items' => $items,
        '#more_url' => $this->getLanguagePrefix() . 'library/browse/by/' . $this->taxonomyTerm,
