...
namespace Drupal\issue_events\Plugin\Block;

use Drupal\landportal_remote_block\Plugin\RemoteBlockBase;
use Drupal\taxonomy\Entity\Term;

/**
 * Provides a 'Issue events' block.
 *
 * @Block(
 *   id = "issue_events_block",
 *   admin_label = @Translation("Issue events block"),
 *   category = @Translation("Landportal Remote")
 * )
 */
class IssueEventsBlock extends RemoteBlockBase {
  ...
  public function build() {
    ...
    if (count($items)) {
      $build = [
        '#theme' => 'issue_events_block',
        '#items' => $items,
        '#more_url' => $this->getLanguagePrefix() . 'issues/' . $this->taxonomyTerm . '/event',
