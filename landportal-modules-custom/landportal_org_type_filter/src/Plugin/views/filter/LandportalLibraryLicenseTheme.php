 ...
 *
 * @ViewsFilter("custom_views_license_resource_theme")
 */
class LandportalLibraryLicenseTheme extends InOperator {
  ...
  public function init(ViewExecutable $view, DisplayPluginBase $display, array &$options = NULL) {
    ...
    $this->valueTitle = t('License of the resource');
    ...
    */
    ...
    if (!empty($values_drop)) {
        ...
        \Drupal::logger('Argument')->notice(serialize($contextual_value));

        $query = \Drupal::entityQuery('node')
            ->condition('status', 1)
            ->condition('type', 'landlibrary_resource') 
            ->condition('field_related_themes', $contextual_value);
        ...
        foreach ($library_nodes_entities as $library_node) {
            $field_values = $library_node->get('field_doc_licencing')->getValue();
    ...
    protected function loadOrganizationNidsFromCsv() {
        $csv_file_path = 'public://csv/organizationtype/libraryLicense.csv';
        $organization_nids = [];
