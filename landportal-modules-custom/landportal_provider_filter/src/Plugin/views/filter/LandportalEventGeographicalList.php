 ...
 *
 * @ViewsFilter("custom_views_event_geo")
 */
class LandportalEventGeographicalList extends InOperator {
    ...
    public function init(ViewExecutable $view, DisplayPluginBase $display, array &$options = NULL){
        ...
        $this->valueTitle = t('Event Geo Count');
    ...
    public function query()
    {
        ...
        if (!empty($values_drop)) {
            ...
            foreach ($values_drop as $value) {
                ...
                $node_query->condition('field_geographical_focus', $value);
    ...
    public function generateOptions() {
        ...
        foreach ($organization_nids as $taxonomy_term_tid) {
            ...
            $query = \Drupal::entityQuery('node')
                ...
                ->condition('status', 1)
                ->condition('field_geographical_focus', $taxonomy_term_tid)
                ->condition('langcode', [
    ...
    protected function loadOrganizationNidsFromCsv() {
        $csv_file_path = 'public://csv/datasetProviderFilter/eventgeo.csv';
        $organization_nids = [];
