...
/**
 * Base class for a Views swiper widget type.
 */
abstract class LpSwiperWidgetTypeBase extends PluginBase implements LpSwiperWidgetTypeInterface {
  ...
  public function checkCompatiblity($swiper) {
    ...
    // corresponding calls value in the swiper.
    foreach ($this->pluginDefinition['accepts'] as $accept_key => $accept_value) {
      if (is_array($accept_value) && !empty($accept_value['required']) && !in_array($accept_key, $swiper['calls'])) {
        $is_compatible = 0;
        break;
      }
    }
    ...
    if ($is_compatible) {
      ...
      foreach ($this->pluginDefinition['calls'] as $calls_key => $calls_value) {
        if (is_array($calls_value) && !empty($calls_value['required']) && !in_array($calls_key, $swiper['accepts'])) {
          $is_compatible = 0;
          break;
        }
      }
